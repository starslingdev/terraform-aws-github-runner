...
| <a name="input_tenant_table_arn"></a> [tenant\_table\_arn](#input\_tenant\_table\_arn) | DynamoDB table ARN for multi-tenant configuration. Required for IAM policies when tenant\_table\_name is set. | `string` | `null` | no |
| <a name="input_tenant_table_name"></a> [tenant\_table\_name](#input\_tenant\_table\_name) | DynamoDB table name for multi-tenant configuration. When set, enables tenant validation in the webhook. | `string` | `null` | no |
